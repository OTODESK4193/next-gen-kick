...
juce::AudioProcessorValueTreeState::ParameterLayout NextGenKickAudioProcessor::createParameterLayout()
{
    ...
    createFloat("subDecay", "Sub Amp Decay", 0.10f, 0.35f, 0.25f);
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>("hqMode", "HQ Mode (Oversampling)", juce::StringArray{ "Standard", "High Quality (2x)" }, 0));
...
void NextGenKickAudioProcessor::initPresets() {
    auto add = [&](juce::String n, int aw, float al, float ad, float ac, float at, float ah, float ap,
        ...
        int sat, float dr, float mo, float mw) {
            PresetData p;
            p.name = n;
            ...
            p.satType = sat; p.hqMode = false; p.mDrive = dr; p.mOut = mo; p.mWidth = mw; p.mRelease = 5.0f; p.mPhase = 0; p.limThresh = 0; p.limLook = 1.0f;
            presetList.push_back(p);
        };

    // --- MODERN KICKS ---
    add("Init / Default", 0, 0.4f, 0.02f, 2.0f, 20000, 200, 3000, 0, 0.7f, 350, 43.6f, 0.07f, 1.0f, 0.8f, 0.35f, 1.0f, 5000, false, 29, 0.6f, 0.2f, 4.0f, 0, 1.0f, 0.6f, 1.0f);
    add("TR-808 Pure", 0, 0.3f, 0.02f, 4.0f, 10000, 500, 3000, 0, 0.8f, 180, 48, 0.05f, 3.0f, 0.0f, 0.5f, 0.8f, 3000, false, 36, 0.5f, 0.3f, 3.0f, 0, 1.2f, 0.6f, 0.5f);
    add("TR-909 Punch", 1, 0.5f, 0.02f, 2.5f, 12000, 100, 3000, 4, 0.7f, 300, 50, 0.06f, 1.5f, 0.2f, 0.25f, 2.0f, 8000, false, 38, 0.5f, 0.2f, 4.0f, 3, 2.0f, 0.5f, 0.7f);
    add("TR-606 Box", 3, 0.3f, 0.01f, 5.0f, 8000, 300, 3000, 3, 0.6f, 200, 55, 0.04f, 1.0f, 0.0f, 0.2f, 1.5f, 4000, false, 43, 0.4f, 0.2f, 3.0f, 0, 1.2f, 0.6f, 0.3f);
    add("Analog Fat", 7, 0.2f, 0.015f, 2.0f, 20000, 50, 398.0f, 0, 0.8f, 120, 35, 0.15f, 2.20f, 1.5f, 0.50f, 1.25f, 2500, false, 24, 0.7f, 0.25f, 8.40f, 0, 3.0f, 0.5f, 0.6f);
    add("Clean Club", 0, 0.3f, 0.015f, 2.5f, 12000, 200, 3000, 0, 0.75f, 220, 48, 0.08f, 2.0f, 0.1f, 0.4f, 1.1f, 3000, false, 36, 0.4f, 0.2f, 2.5f, 0, 1.3f, 0.6f, 0.8f);
    add("Tight Pop", 6, 0.4f, 0.01f, 3.0f, 15000, 150, 3000, 4, 0.7f, 300, 52, 0.06f, 3.0f, 0.0f, 0.25f, 1.8f, 6000, false, 40, 0.3f, 0.15f, 3.0f, 10, 1.0f, 0.6f, 0.7f);
    add("Soft Layer", 1, 0.2f, 0.03f, 1.5f, 5000, 200, 3000, 1, 0.6f, 180, 45, 0.1f, 1.5f, 0.0f, 0.3f, 1.0f, 1500, false, 33, 0.5f, 0.2f, 2.0f, 0, 1.1f, 0.6f, 0.6f);
    add("Digital Click", 3, 0.3f, 0.005f, 5.0f, 20000, 500, 3000, 0, 0.8f, 250, 55, 0.05f, 4.0f, 0.0f, 0.2f, 2.0f, 5000, false, 41, 0.2f, 0.1f, 4.0f, 1, 1.5f, 0.6f, 0.8f);
    add("Solid Sub", 0, 0.1f, 0.01f, 2.0f, 3000, 100, 3000, 0, 0.8f, 150, 38, 0.05f, 2.0f, 0.0f, 0.4f, 0.8f, 1000, false, 28, 0.6f, 0.3f, 2.5f, 0, 1.5f, 0.6f, 0.5f);

    add("Deep House", 1, 0.2f, 0.03f, 1.5f, 5000, 300, 3000, 0, 0.7f, 180, 48, 0.08f, 2.0f, 0.5f, 0.3f, 1.0f, 1500, false, 36, 0.5f, 0.25f, 3.0f, 3, 1.5f, 0.6f, 0.8f);
    add("Tech House", 6, 0.4f, 0.01f, 3.0f, 18000, 150, 3000, 4, 0.8f, 400, 52, 0.05f, 4.0f, 0.1f, 0.2f, 3.0f, 5000, false, 40, 0.4f, 0.2f, 5.0f, 0, 2.0f, 0.5f, 0.9f);
    add("Techno Rumble", 2, 0.2f, 0.05f, 1.0f, 2000, 500, 3000, 0, 0.6f, 250, 45, 0.06f, 2.5f, 0.0f, 0.2f, 1.5f, 800, false, 33, 0.7f, 0.3f, 2.0f, 3, 4.0f, 0.5f, 1.0f);
    add("Warehouse", 4, 0.5f, 0.02f, 2.0f, 15000, 100, 3000, 1, 0.7f, 500, 48, 0.1f, 1.5f, 0.5f, 0.3f, 1.2f, 3000, false, 36, 0.5f, 0.25f, 2.0f, 6, 3.0f, 0.5f, 0.8f);
    add("Minimal", 7, 0.6f, 0.01f, 8.0f, 20000, 1000, 3000, 0, 0.5f, 150, 50, 0.04f, 3.0f, 0.0f, 0.15f, 2.0f, 2000, false, 38, 0.3f, 0.15f, 3.0f, 10, 1.0f, 0.6f, 0.5f);
    add("Dub Techno", 2, 0.1f, 0.05f, 1.0f, 800, 200, 3000, 0, 0.6f, 100, 38, 0.2f, 0.8f, 2.0f, 0.6f, 0.5f, 600, false, 26, 0.7f, 0.35f, 2.0f, 3, 2.0f, 0.5f, 1.0f);
    add("Disco Pop", 1, 0.3f, 0.02f, 2.0f, 10000, 200, 3000, 1, 0.7f, 220, 55, 0.09f, 2.0f, 0.2f, 0.35f, 1.5f, 4000, false, 41, 0.4f, 0.25f, 3.0f, 4, 1.5f, 0.6f, 0.7f);
    add("Future House", 3, 0.5f, 0.015f, 4.0f, 18000, 150, 3000, 0, 0.8f, 450, 58, 0.06f, 3.5f, 0.0f, 0.25f, 2.5f, 8000, false, 46, 0.3f, 0.2f, 4.0f, 1, 2.0f, 0.6f, 0.9f);
    add("Lo-Fi HipHop", 2, 0.6f, 0.04f, 1.0f, 3000, 50, 3000, 0, 0.5f, 120, 45, 0.12f, 1.0f, 0.5f, 0.3f, 1.0f, 800, false, 33, 0.5f, 0.3f, 2.0f, 8, 1.0f, 0.7f, 0.4f);
    add("Acid Kick", 4, 0.4f, 0.01f, 5.0f, 20000, 100, 3000, 2, 0.7f, 600, 50, 0.08f, 4.0f, 0.0f, 0.2f, 2.0f, 12000, false, 38, 0.4f, 0.2f, 3.0f, 5, 3.5f, 0.5f, 0.8f);

    add("Big Room", 4, 0.5f, 0.01f, 6.0f, 20000, 200, 3000, 0, 0.8f, 800, 48, 0.15f, 3.0f, 0.0f, 0.4f, 1.0f, 12000, false, 36, 0.2f, 0.3f, 2.0f, 1, 2.5f, 0.5f, 1.0f);
    add("Prog Trance", 0, 0.4f, 0.02f, 3.0f, 15000, 300, 3000, 0, 0.7f, 350, 50, 0.08f, 2.5f, 0.1f, 0.3f, 1.8f, 6000, false, 38, 0.4f, 0.25f, 3.0f, 0, 1.5f, 0.6f, 0.8f);
    add("Psytrance", 7, 0.6f, 0.005f, 10.0f, 20000, 500, 3000, 4, 0.8f, 500, 55, 0.04f, 5.0f, 0.0f, 0.15f, 3.0f, 12000, false, 43, 0.1f, 0.15f, 5.0f, 5, 2.0f, 0.6f, 0.7f);
    add("Uplifting", 0, 0.5f, 0.02f, 4.0f, 18000, 400, 3000, 0, 0.7f, 400, 52, 0.07f, 3.0f, 0.0f, 0.25f, 2.0f, 8000, false, 40, 0.3f, 0.25f, 3.5f, 9, 2.0f, 0.6f, 0.9f);
    add("Melbourne", 3, 0.4f, 0.03f, 2.0f, 12000, 100, 3000, 2, 0.8f, 300, 50, 0.1f, 2.0f, 0.5f, 0.3f, 1.5f, 10000, false, 38, 0.3f, 0.25f, 2.0f, 4, 2.5f, 0.5f, 0.9f);
    add("Complextro", 3, 0.5f, 0.02f, 3.0f, 15000, 200, 3000, 3, 0.7f, 400, 45, 0.08f, 2.5f, 0.2f, 0.25f, 2.0f, 12000, false, 33, 0.4f, 0.2f, 3.0f, 8, 6.0f, 0.4f, 0.8f);
    add("Eurodance", 0, 0.4f, 0.02f, 3.0f, 20000, 300, 3000, 0, 0.8f, 300, 55, 0.06f, 2.0f, 0.0f, 0.2f, 1.5f, 8000, false, 41, 0.3f, 0.2f, 3.0f, 0, 1.2f, 0.6f, 0.7f);
    add("Hard Trance", 2, 0.5f, 0.02f, 3.0f, 15000, 200, 3000, 2, 0.8f, 500, 50, 0.1f, 2.5f, 0.1f, 0.3f, 1.5f, 12000, false, 38, 0.3f, 0.25f, 3.0f, 5, 3.0f, 0.5f, 0.9f);
    add("Tropical", 5, 0.3f, 0.02f, 2.0f, 8000, 100, 3000, 1, 0.7f, 180, 48, 0.06f, 1.5f, 0.0f, 0.2f, 1.2f, 4000, false, 36, 0.4f, 0.2f, 2.0f, 2, 1.2f, 0.6f, 0.7f);
    add("Future Rave", 4, 0.6f, 0.01f, 5.0f, 20000, 200, 3000, 0, 0.8f, 600, 58, 0.08f, 3.0f, 0.0f, 0.25f, 1.8f, 10000, false, 46, 0.2f, 0.2f, 3.0f, 1, 2.5f, 0.5f, 0.8f);

    add("Trap 808 Long", 0, 0.2f, 0.02f, 5.0f, 10000, 500, 3000, 0, 0.9f, 200, 40, 0.08f, 3.0f, 0.0f, 0.5f, 0.5f, 2000, true, 36, 0.0f, 0.3f, 1.0f, 0, 2.0f, 0.6f, 0.5f);
    add("Trap 808 Hard", 4, 0.4f, 0.01f, 4.0f, 15000, 300, 3000, 0, 0.9f, 300, 45, 0.06f, 4.0f, 0.2f, 0.4f, 0.8f, 5000, true, 40, 0.0f, 0.3f, 2.0f, 1, 4.0f, 0.5f, 0.6f);
    add("Drill 808", 0, 0.2f, 0.02f, 3.0f, 12000, 400, 3000, 0, 0.9f, 200, 48, 0.3f, 1.5f, 4.0f, 0.5f, 0.6f, 1500, true, 41, 0.0f, 0.3f, 1.0f, 3, 3.0f, 0.6f, 0.6f);
    add("Dubstep", 6, 0.5f, 0.01f, 6.0f, 20000, 100, 3000, 3, 0.8f, 400, 50, 0.08f, 3.0f, 0.0f, 0.25f, 2.0f, 8000, false, 38, 0.4f, 0.2f, 3.0f, 6, 4.0f, 0.5f, 0.9f);
    add("Hybrid Trap", 2, 0.4f, 0.02f, 3.0f, 10000, 200, 3000, 2, 0.8f, 350, 42, 0.12f, 2.5f, 0.5f, 0.3f, 1.0f, 5000, true, 30, 0.3f, 0.25f, 2.0f, 7, 3.0f, 0.5f, 0.8f);
    add("Future Bass", 1, 0.3f, 0.03f, 1.5f, 8000, 200, 3000, 0, 0.7f, 200, 50, 0.1f, 2.0f, 0.0f, 0.2f, 1.2f, 3000, true, 38, 0.4f, 0.25f, 2.0f, 2, 1.5f, 0.6f, 0.7f);
    add("Phonk", 2, 0.5f, 0.02f, 2.0f, 5000, 100, 3000, 3, 0.8f, 250, 45, 0.1f, 1.5f, 0.0f, 0.3f, 1.0f, 1000, false, 33, 0.3f, 0.3f, 1.5f, 8, 8.0f, 0.4f, 0.5f);
    add("Glitch Hop", 3, 0.4f, 0.01f, 4.0f, 12000, 200, 3000, 1, 0.7f, 300, 50, 0.07f, 3.0f, 0.2f, 0.2f, 1.8f, 4000, false, 38, 0.4f, 0.2f, 3.0f, 7, 3.0f, 0.5f, 0.9f);
    add("UK Garage", 5, 0.3f, 0.015f, 3.0f, 10000, 300, 3000, 0, 0.7f, 200, 55, 0.06f, 2.0f, 0.0f, 0.2f, 1.5f, 3000, false, 43, 0.4f, 0.2f, 3.0f, 0, 1.2f, 0.6f, 0.8f);
    add("Bass House", 7, 0.5f, 0.01f, 5.0f, 15000, 200, 3000, 1, 0.8f, 400, 48, 0.08f, 3.0f, 0.0f, 0.25f, 1.5f, 8000, false, 36, 0.3f, 0.25f, 3.0f, 5, 4.0f, 0.5f, 0.9f);

    add("Hardstyle", 4, 0.6f, 0.01f, 6.0f, 20000, 100, 3000, 2, 0.9f, 800, 55, 0.15f, 4.0f, 0.0f, 0.3f, 2.81f, 15000, false, 43, 0.1f, 0.3f, 4.0f, 7, 1.20f, 0.6f, 1.0f);
    add("Rawstyle", 4, 0.7f, 0.01f, 8.0f, 20000, 50, 3000, 2, 0.9f, 900, 50, 0.2f, 5.0f, 0.0f, 0.4f, 0.8f, 15000, false, 38, 0.0f, 0.3f, 5.0f, 1, 10.0f, 0.4f, 1.0f);
    add("Gabber", 3, 0.5f, 0.01f, 5.0f, 15000, 200, 3000, 3, 0.8f, 600, 55, 0.12f, 3.0f, 0.2f, 0.3f, 2.96f, 10000, false, 43, 0.2f, 0.25f, 3.0f, 6, 12.0f, 0.4f, 0.9f);
    add("Frenchcore", 4, 0.5f, 0.005f, 6.0f, 18000, 300, 3000, 0, 0.8f, 500, 60, 0.08f, 4.0f, 0.0f, 0.25f, 1.5f, 8000, false, 48, 0.3f, 0.2f, 4.0f, 9, 3.0f, 0.5f, 0.8f);
    add("UK Hardcore", 7, 0.4f, 0.01f, 4.0f, 15000, 400, 3000, 0, 0.8f, 400, 65, 0.07f, 3.0f, 0.0f, 0.2f, 1.8f, 6000, false, 53, 0.3f, 0.2f, 3.0f, 0, 2.0f, 0.6f, 0.8f);
    add("Industrial", 2, 0.5f, 0.05f, 1.0f, 5000, 100, 3000, 3, 0.8f, 300, 35, 0.3f, 1.0f, 2.0f, 0.4f, 0.5f, 2000, false, 24, 0.4f, 0.3f, 1.0f, 8, 8.0f, 0.4f, 1.0f);
    add("Neurofunk", 4, 0.5f, 0.01f, 5.0f, 18000, 200, 3000, 2, 0.8f, 500, 50, 0.08f, 3.5f, 0.5f, 0.2f, 1.8f, 10000, false, 38, 0.3f, 0.2f, 3.5f, 6, 4.0f, 0.5f, 0.9f);
    add("DnB Roll", 6, 0.4f, 0.01f, 4.0f, 15000, 300, 3000, 0, 0.8f, 350, 55, 0.06f, 3.0f, 0.0f, 0.15f, 2.0f, 6000, false, 41, 0.3f, 0.2f, 3.0f, 4, 2.0f, 0.6f, 0.8f);
    add("Jump Up", 3, 0.4f, 0.015f, 3.0f, 12000, 150, 3000, 3, 0.7f, 400, 58, 0.06f, 2.5f, 0.0f, 0.15f, 1.5f, 5000, false, 46, 0.4f, 0.2f, 3.0f, 1, 3.0f, 0.6f, 0.8f);
    add("Liquid DnB", 1, 0.3f, 0.02f, 2.0f, 8000, 200, 3000, 0, 0.7f, 200, 52, 0.07f, 2.0f, 0.0f, 0.2f, 1.2f, 3000, false, 40, 0.4f, 0.2f, 2.0f, 2, 1.5f, 0.6f, 0.7f);

    // --- VINTAGE & DISCO ---
    add("70s Disco", 5, 0.4f, 0.04f, 1.0f, 5000, 100, 3000, 4, 0.7f, 180, 55, 0.06f, 1.2f, 0.0f, 0.15f, 1.0f, 2000, false, 36, 0.0f, 0.15f, 2.0f, 0, 1.05f, 0.6f, 0.8f);
    add("70s Funk", 3, 0.3f, 0.03f, 1.5f, 6000, 150, 3000, 3, 0.6f, 150, 52, 0.08f, 1.5f, 0.1f, 0.2f, 1.0f, 1500, false, 33, 0.0f, 0.2f, 2.0f, 2, 1.1f, 0.6f, 0.7f);
    add("80s SynthPop", 7, 0.5f, 0.01f, 2.0f, 12000, 100, 3000, 0, 0.7f, 250, 48, 0.1f, 2.5f, 0.0f, 0.3f, 1.2f, 4000, false, 38, 0.0f, 0.3f, 3.0f, 1, 1.2f, 0.6f, 0.6f);
    add("80s NewWave", 6, 0.4f, 0.02f, 3.0f, 10000, 200, 3000, 2, 0.7f, 220, 50, 0.09f, 2.0f, 0.2f, 0.25f, 1.0f, 3000, false, 40, 0.0f, 0.2f, 2.5f, 4, 1.1f, 0.6f, 0.7f);
    add("90s HipHop", 1, 0.2f, 0.05f, 1.2f, 3000, 50, 3000, 0, 0.8f, 120, 42, 0.2f, 1.0f, 0.5f, 0.6f, 0.8f, 800, false, 30, 0.0f, 0.4f, 2.0f, 3, 1.5f, 0.6f, 0.5f);
    add("90s House", 0, 0.4f, 0.02f, 2.0f, 15000, 100, 3000, 1, 0.7f, 200, 52, 0.07f, 2.2f, 0.1f, 0.3f, 1.5f, 4000, false, 36, 0.0f, 0.25f, 3.0f, 0, 1.1f, 0.6f, 0.8f);
    add("Motown", 2, 0.3f, 0.06f, 1.0f, 2000, 100, 3000, 1, 0.6f, 100, 45, 0.05f, 1.0f, 0.0f, 0.15f, 1.0f, 1000, false, 33, 0.0f, 0.2f, 2.0f, 2, 1.0f, 0.6f, 0.5f);
    add("Italo Disco", 4, 0.5f, 0.015f, 3.0f, 18000, 100, 3000, 2, 0.7f, 300, 55, 0.08f, 2.5f, 0.0f, 0.3f, 1.2f, 6000, false, 41, 0.0f, 0.2f, 3.0f, 0, 1.2f, 0.6f, 0.8f);
    add("LinnDrum", 0, 0.4f, 0.01f, 4.0f, 8000, 200, 3000, 3, 0.7f, 180, 48, 0.05f, 1.5f, 0.0f, 0.2f, 1.5f, 3000, false, 36, 0.0f, 0.1f, 3.0f, 8, 1.5f, 0.6f, 0.4f);
    add("DMX Sim", 1, 0.5f, 0.01f, 3.0f, 10000, 150, 3000, 4, 0.7f, 200, 50, 0.06f, 1.8f, 0.1f, 0.25f, 1.2f, 2500, false, 38, 0.0f, 0.15f, 3.0f, 1, 1.3f, 0.6f, 0.5f);
    add("CR-78 Soft", 5, 0.3f, 0.02f, 2.0f, 5000, 100, 3000, 0, 0.6f, 120, 55, 0.04f, 1.2f, 0.0f, 0.15f, 1.0f, 1500, false, 41, 0.0f, 0.1f, 2.0f, 0, 1.0f, 0.6f, 0.6f);
    add("Breakbeat", 2, 0.5f, 0.03f, 1.5f, 6000, 80, 3000, 2, 0.7f, 250, 45, 0.1f, 1.8f, 0.2f, 0.35f, 1.2f, 2000, false, 33, 0.0f, 0.2f, 2.5f, 3, 3.0f, 0.6f, 0.8f);
    add("Jungle", 0, 0.2f, 0.01f, 2.0f, 10000, 300, 3000, 0, 0.8f, 150, 40, 0.15f, 2.0f, 0.0f, 0.6f, 1.0f, 1200, false, 28, 0.0f, 0.4f, 2.0f, 2, 1.5f, 0.6f, 0.7f);
    add("Chicago", 1, 0.4f, 0.02f, 2.0f, 12000, 200, 3000, 1, 0.7f, 220, 50, 0.07f, 2.0f, 0.1f, 0.25f, 1.2f, 4000, false, 38, 0.0f, 0.2f, 3.0f, 4, 1.5f, 0.6f, 0.7f);
    add("Detroit", 3, 0.5f, 0.01f, 3.0f, 15000, 150, 3000, 3, 0.7f, 300, 52, 0.05f, 2.5f, 0.2f, 0.2f, 1.5f, 6000, false, 40, 0.0f, 0.15f, 3.0f, 6, 2.0f, 0.5f, 0.8f);
    add("G-Funk", 7, 0.3f, 0.01f, 5.0f, 18000, 500, 3000, 0, 0.7f, 200, 60, 0.06f, 2.0f, 0.0f, 0.3f, 1.2f, 3000, false, 48, 0.0f, 0.2f, 3.0f, 0, 1.1f, 0.6f, 0.6f);
    add("Miami", 0, 0.2f, 0.01f, 3.0f, 10000, 300, 3000, 0, 0.9f, 400, 45, 0.15f, 3.0f, 0.0f, 0.7f, 0.8f, 2000, false, 36, 0.0f, 0.5f, 2.0f, 3, 1.2f, 0.7f, 0.6f);
    add("Speed Garage", 5, 0.4f, 0.02f, 2.0f, 8000, 150, 3000, 0, 0.7f, 200, 50, 0.08f, 2.0f, 0.1f, 0.3f, 1.5f, 3000, false, 38, 0.0f, 0.25f, 3.0f, 0, 1.3f, 0.6f, 0.8f);
    add("Electroclash", 4, 0.6f, 0.01f, 5.0f, 18000, 200, 3000, 3, 0.7f, 350, 55, 0.06f, 3.0f, 0.0f, 0.2f, 1.5f, 8000, false, 41, 0.0f, 0.15f, 4.0f, 8, 3.0f, 0.5f, 0.7f);
    add("Minimal 00s", 7, 0.5f, 0.005f, 8.0f, 20000, 800, 3000, 0, 0.6f, 150, 48, 0.04f, 3.0f, 0.0f, 0.15f, 2.0f, 2500, false, 36, 0.0f, 0.1f, 4.0f, 10, 1.0f, 0.6f, 0.5f);

    // --- SNARES ---
    add("Trap Snare", 0, 0.8f, 0.15f, 2.0f, 15000, 300, 3000, 4, 0.5f, 250, 180, 0.05f, 2.0f, 0.0f, 0.15f, 2.0f, 5000, false, 0, 0.0f, 0.1f, 3.0f, 1, 3.0f, 0.6f, 0.8f);
    add("EDM Snare", 0, 0.9f, 0.1f, 1.0f, 20000, 200, 3000, 2, 0.6f, 300, 200, 0.05f, 3.0f, 0.0f, 0.1f, 2.0f, 8000, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.5f, 0.6f, 0.8f);
    add("Analog Snare", 1, 0.7f, 0.2f, 1.5f, 8000, 400, 3000, 4, 0.5f, 220, 160, 0.08f, 1.5f, 0.0f, 0.15f, 1.5f, 3000, false, 0, 0.0f, 0.1f, 3.0f, 2, 1.2f, 0.6f, 0.6f);
    add("Dubstep Clap", 1, 0.8f, 0.12f, 3.0f, 12000, 500, 3000, 4, 0.4f, 250, 200, 0.02f, 4.0f, 0.0f, 0.08f, 3.0f, 6000, false, 0, 0.0f, 0.1f, 3.0f, 8, 4.0f, 0.5f, 0.8f);
    add("Lo-Fi Rim", 2, 0.6f, 0.05f, 1.5f, 4000, 200, 3000, 4, 0.6f, 300, 250, 0.02f, 2.0f, 0.0f, 0.05f, 1.5f, 2000, false, 0, 0.0f, 0.1f, 3.0f, 8, 2.0f, 0.6f, 0.5f);
    add("Piccolo Snare", 0, 0.8f, 0.1f, 2.0f, 18000, 400, 3000, 0, 0.5f, 400, 300, 0.05f, 3.0f, 0.0f, 0.1f, 2.0f, 4000, false, 0, 0.0f, 0.1f, 3.0f, 1, 2.0f, 0.6f, 0.6f);
    add("Hardstyle Snare", 4, 0.9f, 0.2f, 3.0f, 15000, 200, 3000, 2, 0.6f, 250, 180, 0.1f, 2.0f, 0.5f, 0.2f, 1.5f, 8000, false, 0, 0.0f, 0.1f, 3.0f, 7, 5.0f, 0.5f, 0.9f);
    add("DnB Snare", 0, 0.8f, 0.15f, 2.5f, 12000, 300, 3000, 4, 0.5f, 240, 190, 0.06f, 2.5f, 0.0f, 0.12f, 2.0f, 5000, false, 0, 0.0f, 0.1f, 3.0f, 6, 2.5f, 0.6f, 0.8f);
    add("80s Snare", 1, 0.7f, 0.25f, 1.0f, 6000, 200, 3000, 3, 0.5f, 200, 150, 0.1f, 1.5f, 0.0f, 0.2f, 1.2f, 2500, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.2f, 0.6f, 0.7f);
    add("Industrial Snare", 2, 0.9f, 0.2f, 2.0f, 8000, 100, 3000, 2, 0.6f, 220, 180, 0.15f, 1.0f, 2.0f, 0.2f, 1.0f, 2000, false, 0, 0.0f, 0.1f, 3.0f, 8, 8.0f, 0.5f, 0.9f);

    // --- TOMS ---
    add("808 Tom Low", 0, 0.2f, 0.01f, 2.0f, 5000, 100, 3000, 0, 0.8f, 150, 80, 0.2f, 2.0f, 0.0f, 0.3f, 1.5f, 2000, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.0f, 0.6f, 0.6f);
    add("808 Tom Hi", 0, 0.2f, 0.01f, 2.0f, 5000, 100, 3000, 0, 0.8f, 250, 140, 0.15f, 2.0f, 0.0f, 0.25f, 1.5f, 2500, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.0f, 0.6f, 0.6f);
    add("909 Tom", 0, 0.3f, 0.02f, 3.0f, 8000, 100, 3000, 4, 0.7f, 180, 90, 0.15f, 2.5f, 0.0f, 0.3f, 1.2f, 4000, false, 0, 0.0f, 0.1f, 3.0f, 3, 1.2f, 0.6f, 0.7f);
    add("Simmons Tom", 0, 0.4f, 0.05f, 1.0f, 2000, 500, 3000, 4, 0.7f, 200, 60, 0.3f, 1.5f, 0.0f, 0.4f, 1.0f, 1500, false, 0, 0.0f, 0.1f, 3.0f, 2, 1.5f, 0.6f, 0.5f);
    add("Acoustic Tom", 1, 0.3f, 0.03f, 2.0f, 6000, 200, 3000, 1, 0.8f, 160, 85, 0.25f, 2.0f, 0.5f, 0.35f, 1.5f, 3000, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.0f, 0.6f, 0.6f);
    add("Tribal Tom", 1, 0.2f, 0.02f, 2.0f, 4000, 150, 3000, 1, 0.7f, 140, 90, 0.15f, 1.5f, 0.0f, 0.2f, 1.2f, 2000, false, 0, 0.0f, 0.1f, 3.0f, 4, 1.2f, 0.6f, 0.7f);
    add("Synthwave Tom", 0, 0.3f, 0.04f, 1.5f, 3000, 100, 3000, 0, 0.7f, 180, 70, 0.3f, 1.2f, 0.0f, 0.4f, 1.0f, 1200, false, 0, 0.0f, 0.1f, 3.0f, 3, 1.5f, 0.6f, 0.6f);
    add("Distorted Tom", 4, 0.4f, 0.02f, 3.0f, 8000, 100, 3000, 2, 0.8f, 200, 80, 0.2f, 2.5f, 0.2f, 0.3f, 1.5f, 5000, false, 0, 0.0f, 0.1f, 3.0f, 6, 3.0f, 0.5f, 0.8f);
    add("Resonant Blip", 7, 0.2f, 0.01f, 2.0f, 5000, 100, 3000, 1, 0.8f, 300, 150, 0.1f, 3.0f, 0.0f, 0.2f, 2.0f, 8000, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.0f, 0.6f, 0.6f);
    add("Default (Empty)", 0, 0.0f, 0.01f, 2.0f, 20000, 20, 3000, 0, 0.0f, 100, 20, 0.1f, 1.0f, 0.0f, 0.1f, 1.0f, 20000, false, 0, 0.0f, 0.1f, 3.0f, 0, 1.0f, 0.6f, 1.0f);
}
...
void NextGenKickAudioProcessor::loadPreset(int index) {
    ...
    setVal("satType", (float)p.satType); setVal("hqMode", p.hqMode ? 1.0f : 0.0f);
...
float NextGenKickAudioProcessor::processSaturationSample(float x, int type, float drive, float& tapeState) {
    if (drive <= 1.001f) return x;
    float g = x * drive;
    switch (type) {
    case 0: return std::tanh(g); case 1: return juce::jlimit(-1.0f, 1.0f, g);
    case 2: { float v = g + 0.15f; return (v > 0) ? (v / (1.0f + std::abs(v * 0.35f))) - 0.1f : v * 0.85f; }
    case 3: {
        float y = 0.92f * std::tanh(g + 0.08f * tapeState);
        tapeState = y;
        return y;
    }
    case 4: return g / (1.0f + 0.45f * std::abs(g)); case 5: return (std::abs(g) < 1.0f) ? g - (g * g * g) / 3.0f : (g > 0 ? 0.67f : -0.67f);
    case 6: return std::atan(g * 2.2f) * 0.58f; case 7: return std::sin(g * juce::MathConstants<float>::pi);
...
void NextGenKickAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock) {
    ...
    tapeHysteresisL = 0.0f;
    tapeHysteresisR = 0.0f;
...
void NextGenKickAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    const bool hqMode = (int)apvts.getRawParameterValue("hqMode")->load() == 1;
    ...
    for (const auto metadata : midiMessages) {
        if (metadata.getMessage().isNoteOn()) {
            ...
            isNoteActive = true; noteOnTime = 0.0; subAttackCounter = 0;
            ...
            oversampler.reset();
            tapeHysteresisL = 0.0f; tapeHysteresisR = 0.0f;
        }
    }
    ...
    float sNotVal = 0, sDecV = 0, sCurV = 0, sLevV = 0, sPanV = 0, mDriVal = 0, mOutVal = 0, mRelVal = 0, lThrDB = 0;
    ...
    if (hqMode) {
        ...
        for (int ch = 0; ch < 2; ++ch) {
            ...
            float& tapeState = (ch == 0 ? tapeHysteresisL : tapeHysteresisR);
            for (int i = 0; i < (int)upsampledBlock.getNumSamples(); ++i) {
                p[i] = processSaturationSample(p[i], sMod, mDriVal, tapeState);
            }
        }
